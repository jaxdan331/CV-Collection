...
class LoadImagesAndLabels(Dataset):  # for training/testing
    def __init__(self, path, img_size=416, augment=False, base_path=None):
    ...
    def __getitem__(self, index):
        img_path = self.img_files[index] if self.base_path is None else join(self.base_path, self.img_files[index])
        label_path = self.label_files[index] if self.base_path is None else join(self.base_path, self.label_files[index])
        # print(img_path)
        # print(label_path)
...
if __name__ == '__main__':
    ...
    train_path = parse_data_cfg('data/rbc.data')['train']
    trainset = LoadImagesAndLabels(train_path, img_size=416, augment=True)
    trainloader = DataLoader(trainset, batch_size=16, num_workers=0, shuffle=False, collate_fn=trainset.collate_fn)

    for imgs, tar, _, _ in trainloader:
        print(imgs.size())
        print(type(tar))
